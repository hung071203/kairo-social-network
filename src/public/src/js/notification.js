...
const notificationsList = document.querySelector(
  '#notifications-scroll .list-group',
);
  ...
  if (notificationToggle) {
    notificationToggle.addEventListener('click', function () {
      ...
      setTimeout(() => {
        if (
          !document
            .querySelector('.dropdown-notifications .dropdown-menu')
...
function renderNotifications(newNotifications) {
  ...
  if (
    ...
    loadingItems[0].querySelector('.spinner-border')
  ) {
    notificationsList.innerHTML = '';
  }
  ...
  try {
    ...
    if (notification.redirectUrl) {
      if (
        ...
        notification.redirectUrl.startsWith('https://')
      ) {
